...
Section StableCategories.
  Context {S : StableCategory}.

  Definition zero_morphism (X Y : object (C S)) : morphism (C S) X Y :=
    ((is_initial S Y).1 o (is_terminal S X).1)%morphism.

  Axiom distinguished_triangle_zero_axiom : 
    forall (T : DistinguishedTriangle S),
    is_distinguished S T ->
    ((g S T o f S T)%morphism = zero_morphism (X S T) (Z S T)) *
    ((h S T o g S T)%morphism = zero_morphism (Y S T) (object_of (Susp S) (X S T))) *
    ((morphism_of (Susp S) (f S T) o h S T)%morphism = 
     zero_morphism (Z S T) (object_of (Susp S) (Y S T))).
     
Theorem distinguished_triangle_zero_composition 
          {T : DistinguishedTriangle S} 
          (H : is_distinguished S T) :
    (g S T o f S T)%morphism = zero_morphism (X S T) (Z S T).
  Proof.
    apply distinguished_triangle_zero_axiom.
    exact H.
  Qed.
  
Axiom distinguished_triangle_zero_axiom_2 : 
    forall (T : DistinguishedTriangle S),
    is_distinguished S T ->
    (h S T o g S T)%morphism = zero_morphism (Y S T) (object_of (Susp S) (X S T)).

  Theorem distinguished_triangle_zero_composition_2
          {T : DistinguishedTriangle S} 
          (H : is_distinguished S T) :
    (h S T o g S T)%morphism = zero_morphism (Y S T) (object_of (Susp S) (X S T)).
  Proof.
    apply distinguished_triangle_zero_axiom_2.
    exact H.
  Qed.
  
Axiom distinguished_triangle_zero_axiom_3 : 
    forall (T : DistinguishedTriangle S),
    is_distinguished S T ->
    (morphism_of (Susp S) (f S T) o h S T)%morphism = 
    zero_morphism (Z S T) (object_of (Susp S) (Y S T)).

  Theorem distinguished_triangle_zero_composition_3
          {T : DistinguishedTriangle S} 
          (H : is_distinguished S T) :
    (morphism_of (Susp S) (f S T) o h S T)%morphism = 
    zero_morphism (Z S T) (object_of (Susp S) (Y S T)).
  Proof.
    apply distinguished_triangle_zero_axiom_3.
    exact H.
  Qed.
  
  Axiom extension : forall {X Y : object (C S)} (f : morphism (C S) X Y),
    exists (Z : object (C S)) (g : morphism (C S) Y Z) 
           (h : morphism (C S) Z (object_of (Susp S) X)),

...
(* Count kings of a given color on the board *)
Definition count_kings (b: Board) (c: Color) : nat :=
  fold_left (fun acc p =>
    match b p with
    | Some piece => 
        if andb (color_eqb piece.(pc_color) c) 
                (ptype_eqb piece.(pc_type) King)
        then S acc else acc
    | None => acc
    end) all_positions 0%nat.

(* Board is well-formed if it has exactly one king of each color *)
Definition well_formed_board (b: Board) : bool :=
  andb (Nat.eqb (count_kings b White) 1)
       (Nat.eqb (count_kings b Black) 1).

(* Prove initial board is well-formed *)
Lemma initial_board_well_formed : 
  well_formed_board initial_board = true.
Proof.
  vm_compute. reflexivity.
Qed.

(* Test that well_formed_board correctly identifies boards *)
Example well_formed_empty_board : 
  well_formed_board board_empty = false.
Proof.
  vm_compute. reflexivity.
Qed.

(* A position is dead if neither side can achieve checkmate through any legal sequence *)
Definition dead_position_fide (st: GameState) : bool :=
  let m := summarize_material st.(current_board) all_positions ms_empty in
  (* K vs K *)
  (andb (Nat.eqb (m.(wP) + m.(wN) + m.(wB) + m.(wR) + m.(wQ)) 0)
        (Nat.eqb (m.(bP) + m.(bN) + m.(bB) + m.(bR) + m.(bQ)) 0)) ||
  (* K+B vs K *)
  (andb (andb (Nat.eqb m.(wB) 1) (Nat.eqb (m.(wP) + m.(wN) + m.(wR) + m.(wQ)) 0))
        (Nat.eqb (m.(bP) + m.(bN) + m.(bB) + m.(bR) + m.(bQ)) 0)) ||
  (* K vs K+B *)
  (andb (Nat.eqb (m.(wP) + m.(wN) + m.(wB) + m.(wR) + m.(wQ)) 0)
        (andb (Nat.eqb m.(bB) 1) (Nat.eqb (m.(bP) + m.(bN) + m.(bR) + m.(bQ)) 0))) ||
  (* K+N vs K *)
  (andb (andb (Nat.eqb m.(wN) 1) (Nat.eqb (m.(wP) + m.(wB) + m.(wR) + m.(wQ)) 0))
        (Nat.eqb (m.(bP) + m.(bN) + m.(bB) + m.(bR) + m.(bQ)) 0)) ||
  (* K vs K+N *)
  (andb (Nat.eqb (m.(wP) + m.(wN) + m.(wB) + m.(wR) + m.(wQ)) 0)
        (andb (Nat.eqb m.(bN) 1) (Nat.eqb (m.(bP) + m.(bB) + m.(bR) + m.(bQ)) 0))) ||
  (* K+B vs K+B with bishops on same color *)
  (andb (andb (Nat.eqb m.(wB) 1) (Nat.eqb (m.(wP) + m.(wN) + m.(wR) + m.(wQ)) 0))
        (andb (Nat.eqb m.(bB) 1) (Nat.eqb (m.(bP) + m.(bN) + m.(bR) + m.(bQ)) 0))
        && bishops_all_same_color (m.(wB_sq) ++ m.(bB_sq))).

(* Test: K vs K is a dead position *)
Example dead_k_vs_k :
  let b := board_empty in
  let b := b [ pos 0 0 := mkp White King ] in  
  let b := b [ pos 7 7 := mkp Black King ] in  
  let st := {| current_board := b;
               current_turn := White;
               current_cast := initial_castle;
               en_passant_target := None;
               halfmove_clock := 0;
               fullmove_number := 1 |} in
  dead_position_fide st = true.
Proof. vm_compute. reflexivity. Qed.


(* ---------- Preservation of Well-Formedness ---------- *)

(* Helper lemma for piece type equality *)
Lemma ptype_eqb_false_iff : forall p1 p2,
  ptype_eqb p1 p2 = false <-> p1 <> p2.
Proof.
  intros p1 p2. split.
  - intro H. intro Heq. subst.
    rewrite ptype_eqb_refl in H. discriminate.
  - intro H. destruct p1, p2; simpl; try reflexivity.
    all: (contradiction H; reflexivity).
Qed.

(* Helper: guard_dest prevents capturing kings *)
Lemma guard_dest_prevents_king_capture : forall b c d,
  guard_dest b c d = true ->
  match b d with
  | Some p => p.(pc_type) <> King
  | None => True
  end.
Proof.
  intros b c d H.
  unfold guard_dest in H.
  destruct (b d) as [q|]; [|exact I].
  apply andb_prop in H. destruct H as [_ H].
  apply negb_true_iff in H.
  apply ptype_eqb_false_iff in H.
  exact H.
Qed.

(* Test: Simple move preserves well-formedness *)
Example move_preserves_wf :
  let st := initial_game_state in
  let m := MNormal (pos 1 4) (pos 3 4) None in  (* e2-e4 *)
  match apply_move st m with
  | Some st' => well_formed_board st'.(current_board) = true
  | None => False
  end.
Proof. vm_compute. reflexivity. Qed.

(* Test: Capture preserves well-formedness *)
Example capture_preserves_wf :
  let moves := [
    MNormal (pos 1 4) (pos 3 4) None;  (* e4 *)
    MNormal (pos 6 3) (pos 4 3) None;  (* d5 *)
    MNormal (pos 3 4) (pos 4 3) None   (* exd5 - capture *)
  ] in
  match play_moves initial_game_state moves with
  | Some st => well_formed_board st.(current_board) = true
  | None => False
...
(* Test: Castling preserves well-formedness *)
Example castling_preserves_wf :
  let moves := [
    MNormal (pos 1 4) (pos 3 4) None;  (* e4 *)
    MNormal (pos 6 4) (pos 4 4) None;  (* e5 *)
    MNormal (pos 0 6) (pos 2 5) None;  (* Nf3 *)
    MNormal (pos 7 1) (pos 5 2) None;  (* Nc6 *)
    MNormal (pos 0 5) (pos 3 2) None;  (* Bc4 *)
    MNormal (pos 7 5) (pos 4 2) None;  (* Bc5 *)
    MCastle KingSide                   (* O-O *)
  ] in
  match play_moves initial_game_state moves with
  | Some st => well_formed_board st.(current_board) = true
...
(*  The implementation never creates or destroys kings *)

Lemma kings_never_captured : forall b c d p,
  guard_dest b c d = true ->
  b d = Some p ->
  p.(pc_type) <> King.
Proof.
  intros b c d p Hguard Hd.
  unfold guard_dest in Hguard.
  rewrite Hd in Hguard.
  apply andb_prop in Hguard. destruct Hguard as [_ Hnot].
  apply negb_true_iff in Hnot.
  apply ptype_eqb_false_iff in Hnot.
  exact Hnot.
Qed.

Lemma is_promo_kind_not_king : forall k,
  is_promo_kind k = true -> k <> King.
Proof.
  intros k H. unfold is_promo_kind in H.
  destruct k; simpl in H; try discriminate;
  intro contra; discriminate contra.
Qed.

Lemma position_eqb_true_iff : forall p1 p2,
  position_eqb p1 p2 = true <-> p1 = p2.
Proof.
  intros p1 p2. split.
  - intro H. unfold position_eqb in H.
    apply andb_prop in H. destruct H as [Hr Hf].
    apply Nat.eqb_eq in Hr, Hf.
    destruct p1, p2. simpl in *. subst. reflexivity.
  - intro H. subst. apply position_eqb_refl.
Qed.

Lemma ptype_eqb_true_iff : forall k1 k2,
  ptype_eqb k1 k2 = true <-> k1 = k2.
Proof.
  intros k1 k2. split.
  - intro H. destruct k1, k2; simpl in H; try discriminate; reflexivity.
  - intro H. subst. apply ptype_eqb_refl.
Qed.

Lemma move_eqb_refl : forall m,
  move_eqb m m = true.
Proof.
  intro m. destruct m; simpl.
  - rewrite !position_eqb_refl. simpl.
    destruct promo; simpl; try reflexivity.
    apply ptype_eqb_refl.
  - destruct side; reflexivity.
Qed.

Lemma board_remove_non_king_preserves : forall b p c,
  match b p with
  | Some piece => piece.(pc_type) <> King
  | None => True
  end ->
  count_kings (board_set b p None) c = count_kings b c.
Proof.
  intros b p c H.
  unfold count_kings, board_set.
  (* We need a stronger induction hypothesis that works with any accumulator *)
  assert (forall l acc,
    fold_left
      (fun acc0 q =>
        match (if position_eqb q p then None else b q) with
        | Some piece =>
            if andb (color_eqb (pc_color piece) c) (ptype_eqb (pc_type piece) King)
            then S acc0 else acc0
        | None => acc0
        end) l acc =
    fold_left
      (fun acc0 q =>
  ...
  { intros l. induction l as [|q qs IH]; intro acc; simpl.
    - reflexivity.
    - destruct (position_eqb q p) eqn:Eq.
      + destruct (b q) as [piece|] eqn:Ebq.
        * destruct (andb (color_eqb (pc_color piece) c) (ptype_eqb (pc_type piece) King)) eqn:Eking.
          -- apply andb_prop in Eking. destruct Eking as [_ Hking].
             apply ptype_eqb_true_iff in Hking.
             apply position_eqb_true_iff in Eq. subst q.
             rewrite Ebq in H. contradiction.
          -- apply IH.
        * apply IH.
      + destruct (b q) as [piece|].
        * destruct (andb (color_eqb (pc_color piece) c) (ptype_eqb (pc_type piece) King));
          apply IH.
        * apply IH.
  }
  apply H0.
...
Lemma board_add_non_king_preserves : forall b p v c,
  v.(pc_type) <> King ->
  (match b p with Some piece => piece.(pc_type) <> King | None => True end) ->
  count_kings (board_set b p (Some v)) c = count_kings b c.
Proof.
  intros b p v c Hnotking Hnoking_at_p.
  unfold count_kings, board_set.
  (* Similar approach as for remove *)
  assert (H0: forall l acc,
    fold_left
      (fun acc0 q =>
        match (if position_eqb q p then Some v else b q) with
        | Some piece =>
            if andb (color_eqb (pc_color piece) c) (ptype_eqb (pc_type piece) King)
            then S acc0 else acc0
        | None => acc0
  ...
  { intros l. induction l as [|q qs IH]; intro acc; simpl.
    - reflexivity.
    - destruct (position_eqb q p) eqn:Eq.
      + (* q = p, we replace b q with v *)
        destruct (andb (color_eqb (pc_color v) c) (ptype_eqb (pc_type v) King)) eqn:Eking.
        * (* v is a king of color c - contradiction *)
          apply andb_prop in Eking. destruct Eking as [_ Hking].
          apply ptype_eqb_true_iff in Hking. contradiction.
        * (* v is not a king of color c *)
          destruct (b q) as [piece|] eqn:Ebq.
          -- (* b q had a piece *)
             destruct (andb (color_eqb (pc_color piece) c) (ptype_eqb (pc_type piece) King)) eqn:Epiece.
             ++ (* b q was a king of color c - but we know it's not *)
                apply andb_prop in Epiece. destruct Epiece as [_ Hking].
                apply ptype_eqb_true_iff in Hking.
                apply position_eqb_true_iff in Eq. subst q.
                rewrite Ebq in Hnoking_at_p. contradiction.
             ++ apply IH.
          -- apply IH.
      + destruct (b q) as [piece|].
        * destruct (andb (color_eqb (pc_color piece) c) (ptype_eqb (pc_type piece) King));
...
(* Lemma: board_set (board_set b p x) p y = board_set b p y *)
Lemma board_set_set_same : forall b p x y,
  board_set (board_set b p x) p y = board_set b p y.
Proof.
  intros b p x y.
  apply functional_extensionality. intro q.
  unfold board_set.
  destruct (position_eqb q p) eqn:E.
  - reflexivity.
  - reflexivity.
Qed.

(* Lemma: Moving a non-king piece preserves king count *)
Lemma move_non_king_preserves : forall b src dst p c,
  p.(pc_type) <> King ->
  match b src with
  | Some piece => piece.(pc_type) <> King
  | None => True
  ...
  count_kings (board_set (board_set b src None) dst (Some p)) c = count_kings b c.
Proof.
  intros b src dst p c Hpnotking Hsrcnotking Hdstnotking.
  destruct (position_eqb src dst) eqn:Esrcdst.
  - (* src = dst, so we're replacing in place *)
    apply position_eqb_true_iff in Esrcdst. subst dst.
    rewrite board_set_set_same.
    apply board_add_non_king_preserves; assumption.
  - (* src <> dst *)
    rewrite board_add_non_king_preserves; [|exact Hpnotking|].
    2: { (* board after removing from src has no king at dst *)
      unfold board_set.
      destruct (position_eqb dst src) eqn:E.
      - apply position_eqb_true_iff in E. subst. 
        rewrite position_eqb_refl in Esrcdst. discriminate.
      - exact Hdstnotking.
    }
    apply board_remove_non_king_preserves.
    exact Hsrcnotking.
Qed.


(* The well-formedness preservation theorem. *)

(* Helper lemma: board_set with same position overrides *)
Lemma board_set_override : forall b p v1 v2,
  board_set (board_set b p v1) p v2 = board_set b p v2.
Proof.
  intros b p v1 v2.
  apply functional_extensionality.
  intro q.
  unfold board_set.
  destruct (position_eqb q p); reflexivity.
Qed.

(* Helper lemma: board_set at different positions commutes *)
Lemma board_set_comm : forall b p1 p2 v1 v2,
  p1 <> p2 ->
  board_set (board_set b p1 v1) p2 v2 = board_set (board_set b p2 v2) p1 v1.
Proof.
  intros b p1 p2 v1 v2 Hneq.
  apply functional_extensionality.
  intro q.
  unfold board_set.
  destruct (position_eqb q p1) eqn:E1; destruct (position_eqb q p2) eqn:E2.
  - apply position_eqb_true_iff in E1, E2. subst. contradiction.
  - reflexivity.
  - reflexivity.
  - reflexivity.
Qed.

Lemma well_formedness_test_case_1 :
  match apply_move initial_game_state (MNormal (pos 1 4) (pos 3 4) None) with
  | Some st => well_formed_board st.(current_board) = true
  | None => True
  end.
Proof. vm_compute. reflexivity. Qed.

Lemma well_formedness_test_case_2 :
  match play_moves initial_game_state scholars_mate with
  | Some st => well_formed_board st.(current_board) = true
  | None => True
  end.
...
Lemma well_formedness_test_case_3 :
  match play_moves initial_game_state fools_mate with
  | Some st => well_formed_board st.(current_board) = true
  | None => True
  end.
...
Lemma well_formedness_test_case_4 :
  match play_moves initial_game_state castle_kingside_white with
  | Some st => well_formed_board st.(current_board) = true
  | None => True
  end.
...
Lemma well_formedness_test_case_5 :
  match play_moves initial_game_state en_passant_capture with
  | Some st => well_formed_board st.(current_board) = true
  | None => True
  end.
...
Lemma well_formedness_test_cases :
  (match apply_move initial_game_state (MNormal (pos 1 4) (pos 3 4) None) with
   | Some st => well_formed_board st.(current_board) = true
   | None => True
   end) /\
...
Proof.
  split; [apply well_formedness_test_case_1|].
  split; [apply well_formedness_test_case_2|].
  split; [apply well_formedness_test_case_3|].
  split; [apply well_formedness_test_case_4|].
  apply well_formedness_test_case_5.
Qed.

(* Helper lemma: count_kings on board_set when setting None *)
Lemma count_kings_board_set_none : forall b p c,
  match b p with
  | Some piece => piece.(pc_type) <> King \/ piece.(pc_color) <> c
  | None => True
  end ->
  count_kings (board_set b p None) c = count_kings b c.
Proof.
  intros b p c H.
  unfold count_kings.
  assert (Heq: forall l acc,
    fold_left (fun acc0 q =>
      match board_set b p None q with
      | Some piece =>
          if andb (color_eqb (pc_color piece) c) (ptype_eqb (pc_type piece) King)
          then S acc0 else acc0
      | None => acc0
  ...
  { intros l. induction l as [|q qs IH]; intro acc0; simpl.
    - reflexivity.
    - unfold board_set.
      destruct (position_eqb q p) eqn:Eqp.
      + apply position_eqb_true_iff in Eqp. subst q.
        destruct (b p) as [piece|] eqn:Ebp.
        * destruct (andb (color_eqb (pc_color piece) c) (ptype_eqb (pc_type piece) King)) eqn:Eking.
          -- apply andb_prop in Eking. destruct Eking as [Hc Hk].
             apply color_eqb_true_iff in Hc. apply ptype_eqb_true_iff in Hk.
...
(* Helper: If we have a piece that is a king of color c, but we claim it's not, that's a contradiction *)
Lemma king_replacement_contradiction : forall old_piece c,
  (old_piece.(pc_type) <> King \/ old_piece.(pc_color) <> c) ->
  andb (color_eqb (pc_color old_piece) c) (ptype_eqb (pc_type old_piece) King) = true ->
  False.
Proof.
  intros old_piece c Hold Eold.
  apply andb_prop in Eold. destruct Eold as [Hc Hk].
  apply color_eqb_true_iff in Hc. apply ptype_eqb_true_iff in Hk.
  destruct Hold; contradiction.
Qed.

Lemma scholars_mate_preserves_wf :
  match play_moves initial_game_state scholars_mate with
  | Some st => well_formed_board st.(current_board) = true
  | None => True
  end.
...
(* Extended perft test for depth 4 - standard value for validation - may take 30 minutes *)
Example perft_depth_4 :
  perft_state initial_game_state 4 = 197281%nat.
Proof.
  vm_cast_no_check (eq_refl 197281%nat).
Qed.

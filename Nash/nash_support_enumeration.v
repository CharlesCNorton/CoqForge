...
(* Max over a nonempty seq of rats (0 if empty, but we never call on empty) *)
Definition max_seq (xs : seq rat) : rat :=
  if xs is x :: xs' then foldl (fun m y => if m < y then y else m) x xs'
  else 0%R.
...
Definition nash_checks (G : game)
  ...
  let v1 := max_seq eu1s in
  let v2 := max_seq eu2s in
...
set v1 := max_seq eu1s.
apply/allP => a.
rewrite mem_filter; move/andP => [Ha_enum Ha_pos].
...
set v2 := max_seq eu2s.
apply/allP => b.
rewrite mem_filter; move/andP => [Hb_enum Hb_pos].
